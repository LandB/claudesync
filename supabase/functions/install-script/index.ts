...
function generateBash(supabaseUrl: string, anonKey: string, token: string): string {
  return [
    ...
    '  # SuccessfulExit=false, not KeepAlive=true: the agent exits 0 on failures a',
    '  # restart cannot fix (blocked device, rejected token) so launchd stops',
    '  # instead of hiding the reason inside a crash loop.',
    '  printf \'<key>RunAtLoad</key><true/><key>KeepAlive</key><dict><key>SuccessfulExit</key><false/></dict>\\n\' >> "$PLIST"',
    '  printf \'<key>StandardOutPath</key><string>%s/agent.log</string>\\n\' "$INSTALL_DIR" >> "$PLIST"',
    '  printf \'<key>StandardErrorPath</key><string>%s/agent.log</string>\\n\' "$INSTALL_DIR" >> "$PLIST"',
    ...
    'echo "  Status:  $INSTALL_DIR/agent-status.json  (why the agent stopped, if it did)"',
    'echo ""',
